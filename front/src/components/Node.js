...
import { detailsChanged } from '../actions/nodes';

const Draggable = posed.div({ draggable: true });

function Node({ nodes, id, details, detailsChanged }) {
    // node icon
    ...
    id = parseInt(id, 10);
    let nodeState = nodes.filter(node => node.id == id)[0];
    console.info('Node id: %O, nodeState: %O', id, nodeState);

    let overlay;
    if (nodeState.status === "mining") {
        overlay = <OverlayMining />;
    } else if (nodeState.status === "broadcasting-block") {
        overlay = <OverlayBroadcasting stroke="SpringGreen" />;
    } else if (nodeState.status === "receiving-block") {
        overlay = <OverlayReceiving stroke="LightGreen" />;
    } else if (nodeState.status === "broadcasting-tx") {
        overlay = <OverlayBroadcasting stroke="DeepSkyBlue" />;
    } else if (nodeState.status === "receiving-tx") {
    ...
    return (
        <Draggable className="Draggable">
            <div className="Node" >
                ...
                <div className="NodeName" onClick={() => detailsChanged({ nodeState })} >
                    Name: {nodeState.name}
                </div>
                ...
                <div className="NodeBc">
                    Blockchain: {nodeState.blocks.length}
                </div>
                <div className="NodeTp">
                    Transaction Pool: {nodeState.transactions.length}
                </div>
            </div>
        </Draggable >
    );
...
function mapStateToProps({ nodes, details }) {
    return {
        nodes,
        details,
    };
...
function mapDispatchToProps(dispatch) {
    return {
        detailsChanged(e) {
            dispatch(detailsChanged(e.nodeState.id, e.nodeState.name, e.nodeState.blocks, e.nodeState.transactions));
        }
    };
}
